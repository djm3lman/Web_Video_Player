 ...
 * @author student
 */
@WebServlet(name = "DownloadFilm", urlPatterns = {"/DownloadFilm"})
public class DownloadFilm extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        String data =getFilmData(filmId);
        String name =getFilmName(filmId);
    ...
    private String getFilmData(java.lang.String filmId) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        ws_clients.DataAccessSOAP port = service.getDataAccessSOAPPort();
        return port.getFilmData(filmId);
    }

    private String getFilmName(java.lang.String filmId) {
        // Note that the injected javax.xml.ws.Service reference as well as port objects are not thread safe.
        // If the calling of port operations may lead to race condition some synchronization is required.
        ws_clients.DataAccessSOAP port = service.getDataAccessSOAPPort();
        return port.getFilmName(filmId);
    }
